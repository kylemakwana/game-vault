"""Load Pydantic catalogue models from JSON resources."""
...
def load_catalog(path: Path, model: type[T]) -> list[T]:
    """Load and validate catalogue records from a JSON file.

    :param path: Path to a JSON array of model objects.
    :param model: Pydantic model used to validate each object.
    :return: Validated catalogue records in source order.
    :raises OSError: If the catalogue file cannot be read.
